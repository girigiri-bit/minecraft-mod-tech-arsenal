...
- **本体を隠した後も残る「手」(両視点共通)** — 本体を消したら、その奥にあった描画が露出した。
  ...
  - **壁掛けモニターの手 = フィードに描画される三人称プレイヤー本体の腕/持ち物**。フィード
    キャプチャ中は`onRenderLevelStage`が自機を「自分を映す」目的で描画しており、カメラが近いと
    腕/持ち物が大写しになる。当初疑ったメインバッファ残留(clear案)は的外れ(手はキャプチャ中に
    新規描画されるためclearでは消えない)→ **撤去**。**修正**: `onRenderPlayer`の条件を
    `remoteActive || FeedManager.isCapturing()`に拡張し、フィードキャプチャ中も自機描画を
    キャンセル。他エンティティ(他プレイヤー/Mob)は通常通り映る。
